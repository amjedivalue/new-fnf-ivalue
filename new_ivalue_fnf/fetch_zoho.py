...
def fetch_zoho_doc():
    ...
    if get_all_sigend_fnf:
        for custody in get_all_sigend_fnf:
            zoho = Zoho_api(doctype = 'Full and Final Statement', name = custody.name, status_child_table_name = "Custody Status", zoho_id = custody.zoho_id, docstatus= custody.docstatus)
...
def update_workfow_status(name, zoho_status):
    ...
    if zoho_status == "completed":
        ...
        frappe.db.set_value('Full and Final Statement', name, 'workflow_state', 'Pending CEO')
